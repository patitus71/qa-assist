...
function ManagerDashboard({ name }: { name: string }) {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (c.sendTime) setSendTime(c.sendTime)
      if (c.recipients) setRecipients(c.recipients)
      if (c.lastSent) setLastSent(c.lastSent)
    }
  }, [])

  function saveConfig(update: { sendTime?: string; recipients?: string[]; lastSent?: string }) {
    const current = JSON.parse(localStorage.getItem('report_config') ?? '{}')
    localStorage.setItem('report_config', JSON.stringify({ ...current, ...update }))
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-lg font-semibold text-ink-900">Welcome, {name}</h1>
        <p className="text-xs text-ink-500 mt-0.5">Sprint overview</p>
      </div>
      ...
      <div className="bg-white rounded-[12px] border border-ink-200 overflow-hidden">
        <div className="px-4 py-3 border-b border-ink-100">
          <h2 className="text-sm font-semibold text-ink-900">Daily email report</h2>
        </div>
        <div className="p-5 flex flex-col gap-4">
          <div className="flex items-center gap-6">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-ink-700">Send time</label>
              <input
                type="time"
                value={sendTime}
            ...
            {lastSent && (
              <div className="text-xs text-ink-500">
                ...
                <span className="ml-2 bg-green-50 text-success text-[10px] px-2 py-0.5 rounded-full">Delivered</span>
              </div>
            )}
          </div>
